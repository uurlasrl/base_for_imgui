...
void metal_adder::init_with_device(MTL::Device *device){
    ...
    NS::String* filePath = NS::String::string("/Users/domeniconeri/CLionProjects/metal/metal-cpp-examples/add.metallib", NS::UTF8StringEncoding);
    //NS::Error* error;
    auto lib =_device->newLibrary(filePath, &error);
    if(error)
    {
